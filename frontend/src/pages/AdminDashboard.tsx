...
const AdminDashboard: React.FC = () => {
  ...
  const handleUpdateBatchProgress = async (progress: number, status: string) => {
    if (!selectedBatch) return;
    
    try {
      await batchService.updateBatchProgress(selectedBatch._id, progress, status);
      toast.success('Batch progress updated successfully');
      loadAdminData();
    } catch (error: any) {
      toast.error(error.response?.data?.message || 'Failed to update batch progress');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="border-b border-gray-200">
            <nav className="-mb-px flex space-x-8 px-6">
              {[
                ...
                { id: 'projects', label: 'Projects', icon: BookOpen },
                { id: 'batches', label: 'Teams & Progress', icon: Users }
              ].map((tab) => {
                const Icon = tab.icon;
                return (
                  <button
          ...
          <div className="p-6">
            ...
            {activeTab === 'overview' && (
              <div className="space-y-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="bg-gray-50 rounded-lg p-6">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-lg font-semibold text-gray-900">Active Teams</h3>
                      <button
                    ...
                    <div className="space-y-3">
                      {batches.filter(b => b.status === 'active').slice(0, 5).map((batch) => (
                        <div key={batch._id} className="p-3 bg-white rounded-lg hover:shadow-sm transition-shadow duration-200">
                          <div className="flex items-center justify-between mb-2">
                            <p className="font-medium text-gray-900">{batch.name}</p>
                            <StatusBadge status={batch.status} type="batch" />
                          </div>
                ...
                <div className="bg-white rounded-lg shadow p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <button
                      ...
                      className="flex items-center justify-center p-4 border-2 border-dashed border-primary-300 rounded-lg text-primary-600 hover:border-primary-400 hover:bg-primary-50 transition-colors duration-200"
                    >
                    ...
                    <button
                      onClick={() => setActiveTab('applications')}
                      className="flex items-center justify-center p-4 border-2 border-dashed border-yellow-300 rounded-lg text-yellow-600 hover:border-yellow-400 hover:bg-yellow-50 transition-colors duration-200"
                    >
                    ...
                    <button
                      onClick={() => setActiveTab('batches')}
                      className="flex items-center justify-center p-4 border-2 border-dashed border-secondary-300 rounded-lg text-secondary-600 hover:border-secondary-400 hover:bg-secondary-50 transition-colors duration-200"
                    >
                                ...
                                <button
                                  onClick={() => {
                                    ...
                                    setReviewData({
                                      ...
                                      assignedRole: application.preferredRole || 'Developer',
                                      batchName: `${application.project.title}-Team-${Date.now()}`
                                    });
                                    setShowReviewModal(true);
                                  }}
            ...
            {activeTab === 'projects' && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {projects.map((project) => (
                    <div key={project._id} className="bg-white rounded-lg border border-gray-200 p-6 hover:shadow-lg transition-shadow duration-200">
                      <div className="flex items-center justify-between mb-4">
                        <h4 className="text-lg font-semibold text-gray-900 line-clamp-1">{project.title}</h4>
                        <StatusBadge status={project.status} type="project" />
                      </div>
                      <p className="text-gray-600 mb-4 line-clamp-3">{project.description}</p>
                      <div className="space-y-2 mb-4">
                      ...
                      <div className="flex space-x-2">
                        <button
                          ...
                          className="flex-1 bg-primary-100 hover:bg-primary-200 text-primary-700 px-3 py-2 rounded-lg text-sm flex items-center justify-center space-x-1 transition-colors duration-200"
                        >
                        ...
                        <button
                          ...
                          className="flex-1 bg-red-100 hover:bg-red-200 text-red-700 px-3 py-2 rounded-lg text-sm flex items-center justify-center space-x-1 transition-colors duration-200"
                        >
            ...
            {activeTab === 'batches' && (
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  <h3 className="text-lg font-semibold text-gray-900">Team Management & Progress Tracking</h3>
                  <div className="text-sm text-gray-600">
                    {stats.activeBatches} active teams • {stats.totalBatches} total
                  </div>
                </div>
                
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {batches.map((batch) => (
                    <div key={batch._id} className="bg-white rounded-lg border border-gray-200 p-6 hover:shadow-lg transition-shadow duration-200">
                      <div className="flex items-center justify-between mb-4">
                        <h4 className="text-lg font-semibold text-gray-900">{batch.name}</h4>
                        <StatusBadge status={batch.status} type="batch" />
                      </div>
                      ...
                      <div className="mb-4">
                        <div className="flex items-center justify-between text-sm text-gray-600 mb-2">
                          ...
                          <span className="font-bold text-primary-600">{batch.progress}%</span>
                        </div>
                      ...
                      <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            ...
                            <p className="text-gray-600">{new Date(batch.createdAt).toLocaleDateString()}</p>
                          </div>
                          {batch.startDate && (
                            <div>
                              <span className="font-medium text-gray-700">Started:</span>
                              <p className="text-gray-600">{new Date(batch.startDate).toLocaleDateString()}</p>
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex space-x-2">
                        <button
                          ...
                          className="flex-1 bg-secondary-100 hover:bg-secondary-200 text-secondary-700 px-3 py-2 rounded-lg text-sm flex items-center justify-center space-x-1 transition-colors duration-200"
                        >
                          <TrendingUp className="h-4 w-4" />
                          <span>Update Progress</span>
                        </button>
                ...
                {batches.length === 0 && (
                  <div className="text-center py-12">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Teams will be created automatically when you approve student applications
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center space-x-2">
                      <div className="w-8 h-8 bg-primary-500 rounded-full flex items-center justify-center">
                        <span className="text-xs text-white font-medium">
                          {selectedApplication.user.name.charAt(0).toUpperCase()}
                        </span>
                      </div>
                      <div>
                        <p className="font-medium text-gray-900">{selectedApplication.user.name}</p>
                        <p className="text-gray-600">{selectedApplication.user.email}</p>
                      </div>
                    </div>
                    {selectedApplication.user.profile?.skills && selectedApplication.user.profile.skills.length > 0 && (
                      <div className="mt-3">
                        <p className="font-medium text-gray-700 mb-1">Skills:</p>
                        <div className="flex flex-wrap gap-1">
                          {selectedApplication.user.profile.skills.map((skill, index) => (
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <p><span className="font-medium">Title:</span> {selectedApplication.project.title}</p>
                    <p><span className="font-medium">Difficulty:</span> {selectedApplication.project.difficulty}</p>
                    <p><span className="font-medium">Duration:</span> {selectedApplication.project.duration}</p>
                    <p><span className="font-medium">Team Size:</span> {selectedApplication.project.maxTeamSize}</p>
                    <p><span className="font-medium">Applied:</span> {new Date(selectedApplication.createdAt).toLocaleDateString()}</p>
                  </div>
                </div>
              </div>
              ...
              {/* Application Details */}
              <div>
                <h4 className="font-medium text-gray-900 mb-3">Student's Application</h4>
                <div className="space-y-4">
                  <div>
                  ...
                  {selectedApplication.preferredRole && (
                    <div>
                      <p className="text-sm font-medium text-gray-700 mb-2">Preferred Role:</p>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-green-50 rounded-lg">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={reviewData.batchName}
                          onChange={(e) => setReviewData({...reviewData, batchName: e.target.value})}
                          placeholder="e.g., Project-Alpha-Team-1"
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                        />
                      </div>
                    </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setReviewData({...reviewData, reviewNotes: e.target.value})}
                      rows={3}
                      placeholder="Add feedback for the student about your decision..."
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                    />
                  </div>
                </div>
      ...
      <ProjectModal
        ...
        onSave={projectModalMode === 'create' ? handleCreateProject : handleUpdateProject}
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to delete "${selectedProject?.title}"? This action cannot be undone and will affect all related applications.`}
      ...
      <BatchProgressModal
        ...
        onSave={handleUpdateBatchProgress}
        batch={selectedBatch}
      />
    </div>
  );
