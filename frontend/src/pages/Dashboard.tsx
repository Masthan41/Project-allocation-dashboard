...
//           <p className="mt-2 text-gray-600">
//             {user?.role === 'admin' 
//               ? 'Manage projects, applications, and track progress from your admin dashboard.'
//               : 'Track your project applications and collaborate with your team.'
//             }
//           </p>
//         </div>
...
const Dashboard: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      setProjects(results[1]?.projects || []);
      if (user?.currentBatch && results[2]) {
        setMyBatch(results[2]?.batch || null);
      }
      if (user?.role === 'admin' && results[3]) {
        setAllBatches(results[3]?.batches || []);
      }
                ...
                <div className="space-y-4">
                  {applications.slice(0, 5).map((application) => (
                    <div key={application._id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                      <div className="flex-1">
                        <h4 className="font-medium text-gray-900">
                          {application.project?.title || 'Project not assigned'}
                        </h4>
                        {user?.role === 'admin' && (
                          <p className="text-sm text-gray-600">by {application.user?.name || 'Unknown'}</p>
                        )}
                        <p className="text-xs text-gray-500 mt-1">
            ...
            <div className="p-6">
              {user?.role === 'admin' ? (
                  ...
                  <div className="space-y-4">
                    {allBatches.filter(batch => batch.status === 'active').slice(0, 5).map((batch) => (
                      <div key={batch._id} className="p-4 border border-gray-200 rounded-lg">
                        ...
                        <p className="text-sm text-gray-600">{batch.project?.title || 'No project assigned'}</p>
                        <div className="mt-2 flex items-center justify-between">
                          <span className="text-xs text-gray-500">
                            {batch.members?.length || 0}/{batch.maxMembers || 0} members
                          </span>
                          <div className="flex items-center text-xs text-gray-500">
                            <TrendingUp className="h-4 w-4 mr-1" />
                            {batch.progress || 0}%
                ...
                <div className="p-4 border border-gray-200 rounded-lg">
                  ...
                  <p className="text-sm text-gray-600">{myBatch.project?.title || 'No project assigned'}</p>
                  <div className="mt-4">
                    <div className="flex items-center justify-between text-sm text-gray-600 mb-1">
                      <span>Progress</span>
                      <span>{myBatch.progress || 0}%</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className="bg-primary-500 h-2 rounded-full transition-all duration-300"
                        style={{ width: `${myBatch.progress || 0}%` }}
                      ></div>
                    </div>
                  </div>
                  <div className="mt-4">
                    ...
                    <div className="space-y-2">
                      {myBatch.members?.map((member, index) => (
                        <div key={index} className="flex items-center justify-between text-sm">
                          ...
                          <span className="text-gray-500">{member.role || 'N/A'}</span>
                        </div>
